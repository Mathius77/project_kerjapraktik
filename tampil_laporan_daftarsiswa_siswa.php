...
if(isset($_POST['submit'])){
    ...
    if(empty($kelas) || empty($tgl_awal) || empty($tgl_akhir)){
        if(empty($kelas)){
            echo "<font color='red'>Kelas Awal masih kosong.</font><br/>";
        }
        ...
        $result = mysqli_query($conn, "SELECT * FROM view_daftarsiswa WHERE tgl_bayar >'".$tgl_awal."' AND kelas=$kelas AND nis='".$_POST['nis']."'");
        //print_r($result); die;  
        
        echo '<h5 style="text-align:center;">TRANSAKSI PEMBAYARAN SPP</h5>';

...
<head>
    ...
    <title>Rekap Uang Sekolah SD</title>
...
if(isset($_POST['submit'])){
    ...
    if(empty($tgl_awal)){
        ...
        $result = mysqli_query($conn, "SELECT SUM(a.biaya) AS total_uang_sekolah, SUM(a.uang_kegiatan) AS total_uang_kegiatan, SUM(a.ekstrakurikuler) AS total_ekstrakurikuler FROM tbl_lihat_pembayaran a INNER JOIN tbl_tipe_kelas b ON a.kelas=b.kelas WHERE tgl_bayar='".$tgl_awal."' AND tipe='SD'");
        //print_r($result); die;  
        $res = mysqli_fetch_array($result);
        $total = $res['total_uang_sekolah'] + $res['total_uang_kegiatan'] + $res['total_ekstrakurikuler'];
        //print_r($res); die;
        echo '<h5 style="text-align:center;">Rekap Uang Sekolah SD Pamardi Yuwana Bhakti</h5>';
...
</br><p class=kata><strong>Uang Sekolah SD yang disetor = Rp '.$total.'.00</strong></p>';
}
}
?>

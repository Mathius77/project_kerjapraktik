...
<head>
    ...
    <title>Transaksi Pemesanan Seragam</title>
...
if(isset($_POST['submit'])){
    ...
    if(empty($nis)){
        ...
        $result = mysqli_query($conn, "SELECT id, nis, nama, thn_ajar, tingkat, jns_pakaian, ukuran, jumlah, harga, DATE_FORMAT(tgl_bayar, '%d/%m/%Y') AS tgl_bayar_saja FROM v_04 WHERE nis='".$_POST['nis']."'");
        /* mengecek error di $result
        if (!$result) {
            printf("Error: %s\n", mysqli_error($conn));
            exit();
        }*/  
        ...
        echo '<h5 style="text-align:center;">TRANSAKSI PEMESANAN SERAGAM</h5>';

...
<head>
    ...
    <title>Laporan Pembayaran SPP SMP</title>
...
if(isset($_POST['submit'])){
    ...
    if(empty($tgl_awal) || empty($tgl_akhir)){
        ...
        $result = mysqli_query($conn, "SELECT nis, nama, kelas, tipe, bln_bayar, uang_sekolah, uang_kegiatan, ekstrakurikuler, DATE_FORMAT(tgl_bayar, '%d/%m/%Y') as tgl_bayar_saja, total_bayar FROM view_daftarsiswa WHERE tgl_bayar >'".$tgl_awal."' AND tipe='SMP'");
        //print_r($result); die;  
        
        echo '<h5 style="text-align:center;">LAPORAN PEMBAYARAN SPP (SMP)</h5>';
